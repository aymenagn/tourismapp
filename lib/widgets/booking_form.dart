...
class _BookingFormState extends State<BookingForm> {
  ...
  String _getMonthName(int month) {
    ...
    return DateFormat('MMM').format(DateTime(2024, month)); // Use intl to get month name//jdid
    /*switch (month) {
      case 1:
        return 'Jan';
      case 2:
  ...
  void _calculateTotalPrice() {//jdid
    const double basePricePerNight = 100.0; // Base price per night//jdid
    final int numberOfDays = _toDate != null && _fromDate != null//jdid
        ? _toDate!.difference(_fromDate!).inDays//jdid
        : 0;//jdid
    final double roomCost = _rooms * basePricePerNight * numberOfDays;//jdid
    final double adultCost = _adults * 20.0 * numberOfDays; // Example adult price//jdid
    final double childrenCost = _children * 10.0 * numberOfDays; // Example child price//jdid
    setState(() {//jdid
      _totalPrice = roomCost + adultCost + childrenCost;//jdid
      if (_totalPrice < 0) {//jdid
        _totalPrice = 0;//jdid
      }//jdid
    });//jdid
  }//jdid
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: <Widget>[
              const Text('Children (-18yrs)', style: TextStyle(fontSize: 18.0)),
              Row(
                children: <Widget>[
                  IconButton(
                    icon: const Icon(
          ...
          const SizedBox(height: 30),//jdid
            ...
            Row(//jdid
              ...
              children: [//jdid
                Container(//jdid
                  padding: const EdgeInsets.symmetric(vertical: 12, horizontal: 16),//jdid
                  decoration: BoxDecoration(//jdid
                    borderRadius: BorderRadius.circular(10),//jdid
                    color: Colors.grey.shade200, // Light gray background//jdid
                  ),//jdid
                  child: Row(//jdid
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,//jdid
                    children: <Widget>[//jdid
                      const Text(//jdid
                        'Total:',//jdid
                        style: TextStyle(fontSize: 18.0, fontWeight: FontWeight.bold),//jdid
                      ),//jdid
                      Text(//jdid
                        '\$${_totalPrice.toStringAsFixed(2)} USD', // Format to 2 decimal places//jdid
                        style: const TextStyle(fontSize: 18.0, color: Colors.green), // Or any other color//jdid
                      ),//jdid
                    ],//jdid
                  ),//jdid
                ),
      ...
      builder:
          (context) => AlertDialog(
            ...
            content: Text(
              ...
              semanticsLabel: 'Total Price: \$${_totalPrice.toStringAsFixed(2)} USD',//jdid
            ),
            actions: [
              TextButton(
                onPressed: () => Navigator.of(context).pop(),
